	...
	for (y = height; y > 0; y--)		// Colonna
	{
		...
		{	
			...
			for (int s = 0; s < samples; s++)	// Campionamento sul singolo pixel
			{
				...
				col += pixelColorFunction(r, world, 0); // Restituisco il colore di ci� che interseca il raggio nel sub-pixel casuale, il valore viene iterativamente accumulato all'interno del pixel.
			}
...
inline vec3 pixelColorFunction(const ray& r, Geometry* world, int depth)
{
	...
	else // Colorazione di cio che non � un oggetto
	{
